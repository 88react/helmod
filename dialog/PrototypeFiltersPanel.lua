...
function PrototypeFiltersPanel:getContentPanel()
  ...
  -- panel.style.minimal_width = width_main
  panel.style.horizontally_stretchable = true
  local scroll_panel = GuiElement.add(panel, GuiScroll("scroll_panel"):style("helmod_scroll_pane"))
...
function PrototypeFiltersPanel:addRowFilter(itable, prototype_filter, index)
  ...
  if options == "comparison" then
    local comparaison_cell = GuiElement.add(itable, GuiTable("comparison", index):column(2))
    if prototype_filter.option == nil then
      prototype_filter.option = {comparison = comparisons[1], value = 0}
    end
    local comparison = prototype_filter.option.comparison
    local comparison_value = prototype_filter.option.value or 0
  ...
  elseif prototype_filter.filter == "name" or prototype_filter.filter == "mod" then
    local name_cell = GuiElement.add(itable, GuiTable("names", index):column(2))
    if prototype_filter.option == nil then
      prototype_filter.option = ""
    end
    local names_value = prototype_filter.option or ""
  ...
  elseif prototype_filter.filter == "collision-mask" then
    local collision_mask_cell = GuiElement.add(itable, GuiTable("collision-mask", index):column(2))
    if prototype_filter.option == nil then
      prototype_filter.option = {mask = collision_mask[1], mask_mode =collision_mask_mode[1]}
    end
    local mask = prototype_filter.option.mask
    local mask_mode = prototype_filter.option.mask_mode
...
function PrototypeFiltersPanel:updateResult()
  ...
  if table.size(prototype_filters) > 0 then
    ...
    GuiElement.add(data_panel, GuiLabel("info-label"):caption(#elements):style("helmod_label_title_frame"))
...
function PrototypeFiltersPanel:onEvent(event)
  ...
  if event.action == "change-filter-option-value" then
    ...
    if prototype_filter.filter == "name" or prototype_filter.filter == "mod" then
